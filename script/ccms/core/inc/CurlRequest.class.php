...
class CurlRequest {
	...
	*/               
    public function init($params) {
        ...
        //@curl_setopt ( $this -> ch, CURLOPT_FOLLOWLOCATION, 1); < safe mode restrict
        ...
        if (isset($params['login']) & isset($params['password'])) @curl_setopt($this -> ch , CURLOPT_USERPWD,$params['login'].':'.$params['password']);
	...
	function parse_http_headers($rawheaders) {
	    ...
	    foreach($lines as $line) {
	        if (preg_match("/^(.+?):\s+(.+)$/",trim($line),$matches)) $headers[$matches[1]] = $matches[2];
	        unset($matches);
	    }
	    return $headers;
	}
	
	function parse_http_cookies($rawheaders) {
	    $lines = explode("\n",$rawheaders);
	    if(substr($lines[0],0,5) == 'HTTP/') {
	        $response = array_shift($lines);
	    }
		$res=array();
	    foreach($lines as $line) {
	        if (preg_match("/^Set-Cookie:([^;]*)/",trim($line),$match)) $res[]=trim($match[1]);
	    }
	    return $res;
	}
	...
	*/
    public function exec() {
		...
		$result['headers'] = $this->parse_http_headers($result['header']);
		$result['cookies'] = $this->parse_http_cookies($result['header']);
