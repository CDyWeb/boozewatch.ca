...
class ProductGalleryManager extends CCMSDomainManager {

  function __construct() {
  
    parent::__construct("ProductGallery");

    $this->addFieldConfig("name=product_id;type=".CCMSDomainField::FIELDTYPE_FK.";required=1;attributes=table:".DATABASE_PREFIX."product,caption:name,delete:cascade");
    $this->addFieldConfig("name=user_id;type=".CCMSDomainField::FIELDTYPE_FK.";required=0;attributes=table:".DATABASE_PREFIX."user,caption:name,delete:cascade");
    ...
    $this->addFieldConfig("name=enclosure;type=".CCMSDomainField::FIELDTYPE_IMG.";required=1");
    ...
    $this->setEditFields(getConfigItem("ProductGalleryManager.editFields",array('title','enclosure')));

    $this->setFilterFieldName("product_id");
  ...
  public function save($id, $data, &$err) {
    ...
    /**
    if ($res) {
      $product=$this->getFilter();
      executeSql("update ".DATABASE_PREFIX."product set lastmod=now() where id={$product}");
    }
    **/
    return $res;
  }
