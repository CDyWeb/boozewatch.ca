...
class HtmlMimeMail5 {
	...
	public static function mail($from,$to,$subject,$msg,$headers=null,$return_path=null) {
      ...
      $return_path=preg_replace('#^.*\<(.*)\>.*$#','$1',$from);
    } 
    ...
    */
    public function setFrom($from)
    {
        ...
        if (empty($this->return_path)) $this->return_path = preg_replace('#^.*\<(.*)\>.*$#','$1',$from);
    }
    ...
    */
    ...
    {
        if (preg_match('#charset=iso-8859-1#i',$html,$match)) {
          $html=utf8_encode(str_replace($match[0],'#charset=UTF-8#i',$html));
        }
        $this->html = $html;
        ...
        if (empty($this->text)) {
          $this->text = html_entity_decode(trim(strip_tags(preg_replace('/<(head|title|style|script)[^>]*>.*?<\/\\1>/s','',$html))),ENT_QUOTES,'UTF-8');
        }
    ...
    */
    public function send($recipients, $type = 'mail')
    {
      ...
      if (defined('HTMLMIMEMAIL5_TYPE')) $type=HTMLMIMEMAIL5_TYPE;
        ...
        switch ($type) {
            case 'mock-db':
              ...
              $sql=sprintf(
                ...
                dbStr(@$this->headers['Message-ID']),
                dbStr(session_id()),
                dbStr(function_exists('getClientIp')?getClientIp():NULL),
                dbStr(@$this->headers['From']),
                dbStr(implode(', ', $recipients)),
                dbStr(@$this->headers['Subject']),
                dbStr($this->html),
                dbStr($this->output),
                dbStr(json_encode($this->headers)),
                dbStr($this->return_path)
              );
              executeSql($sql);
              break;
            ...
            case 'mail':
                ...
                if (!empty($this->return_path)) {
					...
					_log("output: ".strlen($this->output));
					_log("headers: ".print_r($headers,true));
                }
            ...
            case 'smtp':
                ...
                if (defined('HTMLMIMEMAIL5_SMTP_HOST')) $this->smtp_params['host']=HTMLMIMEMAIL5_SMTP_HOST;
                if (defined('HTMLMIMEMAIL5_SMTP_PORT')) $this->smtp_params['port']=HTMLMIMEMAIL5_SMTP_PORT;
